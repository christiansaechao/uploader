...
export const ItemsProvider = ({ children }) => {
  ...
  const publishToPlatform = async (itemId, platform) => {
    try {
      ...
      const response = await axios.post(`/api/platforms/${platform}/${itemId}`, item)
      ...
      dispatch({ 
        type: 'SET_ERROR', 
        payload: error.response?.data?.error || `Failed to publish to ${platform}` 
      })
      throw error
    }
  }
  ...
  const bulkPublish = async (itemIds, platforms) => {
    try {
      ...
      const response = await axios.post('/api/platforms/bulk', { items, platforms })
      ...
      response.data.results.forEach(result => {
        ...
        if (item) {
          ...
          if (result.platforms.shopify && !result.platforms.shopify.error) {
            ...
            updatedItem.status.shopifyMessage = result.platforms.shopify.message
  ...
  const filteredItems = state.items.filter(item => {
    ...
    const matchesPlatform = state.filters.platform === 'all' ||
