        ...
        for index, row in df.iterrows():
            try:
                ...
                item_data = {
                    ...
                    "images": row_dict.get("images", "").split(",") if row_dict.get("images") else [],
                    "tags": row_dict.get("tags", "").split(",") if row_dict.get("tags") else [],
                    ...
                    "shipping": {
                        "weight": float(row_dict.get("shipping_weight", row_dict.get("weight", 0))),
                        "method": str(row_dict.get("shipping_method", "Standard")).strip(),
                        "cost": float(row_dict.get("shipping_cost", 0))
                    },
                    "platforms": {
                        "ebay": str(row_dict.get("ebay", "false")).lower() in ["true", "1", "yes"],
                        "shopify": str(row_dict.get("shopify", "false")).lower() in ["true", "1", "yes"]
                    }
                }
                
                # Clean up images and tags
                item_data["images"] = [img.strip() for img in item_data["images"] if img.strip()]
                item_data["tags"] = [tag.strip() for tag in item_data["tags"] if tag.strip()]
