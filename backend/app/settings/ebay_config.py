...
class EbaySettings(BaseSettings):
    ...
    EBAY_DEV_ID: str = os.getenv(
        "EBAY_DEV_ID"
    )  # Primarily for Traditional APIs, less so for modern REST OAuth
    ...
    def validate_ebay_config(self):
        required_fields = [
            self.EBAY_APP_ID,
            self.EBAY_CERT_ID,
        ]  # DEV_ID less critical for modern OAuth
        return all(
...
if ebay_settings.validate_ebay_config():
    ...
    print(
        "⚠️ Warning: eBay API configuration incomplete. Check EBAY_APP_ID and EBAY_CERT_ID in your .env file."
    )
