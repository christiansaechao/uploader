...
class Item(BaseModel):
    ...
    dimensions: Dimensions = Field(
        default_factory=Dimensions, description="Item dimensions"
    )
    shipping: Shipping = Field(
        default_factory=Shipping, description="Shipping information"
    )

    # Platform settings
    platforms: Platforms = Field(
        default_factory=Platforms, description="Platform selection"
    )

    # Platform status
    platform_status: Dict[str, PlatformInfo] = Field(
        default_factory=lambda: {"ebay": PlatformInfo(), "shopify": PlatformInfo()},
    ...
    class Config:
        ...
        schema_extra = {
            "example": {
                ...
                "shipping": {"weight": 0.6, "method": "Standard", "cost": 5.99},
                "platforms": {"ebay": True, "shopify": True},
            }
        }
